 ...
 *
 *  UPDATE at 09.07.2015:
 *  	1. clearSitemapFiles() - If for the main sitemap-files the special 
 *  	catalog is allocated, this function will allow to clear it before 
 *  	creation of a new set.
 *
 *  	2. Parameter $index_path will allow to set the separate catalog 
 *  	for the index sitemap-file, for example to place it in a site root.
 */
class Sitemap {
	/**
	 * @var \XMLWriter
	...
	/**
	 * Returns index-path of sitemaps
	 * 
	 * @return string
	 */
	...
	/**
	 *  Надо ли добавлять .gz к страницам в индексном файле?
	 *  @return  boolean
	 */
	public function getGzipStatus() {
		return $this->gzip;
	}

	/**
	 *  Будет ли проводиться архивация или нет.
	 *  @param  boolean  $status 
	 */
	public function setGzipStatus($status) {
		$this->gzip = $status;
		return $this;
	}
	...
	/**
	 * Clear sitemap folder.
	 */
	public function clearSitemapFiles()
	{
		$sPath = $this->getPath();

		if ($sPath != '' || $sPath != '/' || strpos($sPath, '..') === FALSE)
		{
			// CodeIgniter handle.
			$CI =& get_instance();
			...
			// Очистить каталог с sitemap-файлами.
			delete_files($this->getPath(), FALSE, TRUE);

			// Очистить индексный sitemap-файл.
			if (file_exists($this->getIndexPath() . $this->getFilename() . self::INDEX_SUFFIX . self::EXT))
			{
				unlink($this->getIndexPath() . $this->getFilename() . self::INDEX_SUFFIX . self::EXT);
			}
		}
	...
	public function createSitemapIndex($loc, $lastmod = 'Today') {
		...
		$indexwriter->openURI($this->getIndexPath() . $this->getFilename() . self::INDEX_SUFFIX . self::EXT);
		...
		for ($index = 0; $index < $this->getCurrentSitemap(); $index++) {
			...
			$indexwriter->writeElement('loc', $loc . $this->getFilename() . ($index ? self::SEPERATOR . $index : '') . self::EXT . ($this->getGzipStatus() ? self::GZIP_EXT : ''));
...
/* Location: ./application/libraries/Sitemap.php */